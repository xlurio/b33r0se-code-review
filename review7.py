"""
Introducing type hints for better maintainability and helping the intellisense (since
Python is dinamically typed, IDEs and editors can't accurately guess the type of some
values unless you tell it. You do that by using type hints)
"""
...
class LevelState:
    ...
    @property
    def curr_num_of_tries(self) -> int:
        return self.__curr_num_of_tries

    @property
    def have_not_guessed_and_have_tries_left(self) -> bool:
        return self.__have_tries_left and self.__have_user_not_guessed

    @property
    def have_no_tries_left(self) -> bool:
        return not self.__have_tries_left

    @property
    def __have_tries_left(self) -> bool:
        return self.__curr_num_of_tries > 0


class Level:

    def __init__(self, level_introduction: str, max_guess: int) -> None:
    ...
    def __ask_for_the_next_guess(self) -> LevelState:
        try:
    ...
    def __check_user_guess(self) -> LevelState:
...
class Game:
    ...
    def __ask_if_player_wants_to_play(self) -> None:
        print(
